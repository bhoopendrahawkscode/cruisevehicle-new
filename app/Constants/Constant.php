...
class Constant
{
    const COMMUNITY_FOLDER = '/uploads/communities/';
    const ATTACHMENT_FOLDER = '/uploads/attachments/';
    const DISK = 's3';
    const PUBLIC_FOLDER = 'public/';
    const OPERATION_TYPE = 'resizeWithPad';
    const NO_IMAGE = '/assets/images/no-image.png';
    const VALIDATION_REQUIRED = 'validations.required';
    const ALPHA_NUM = 'validations.alpha_num';
    const VALIDATION_MIN = 'validations.min';
    const VALIDATION_MAX = 'validations.max';
    const ATTRIBUTE = ':attribute';
    const VALIDATION_EMAIL = 'validations.email';
    const VALIDATION_UNIQUE = 'validations.unique';
    const PASSWORD_REGEX = "validations.password_regex";
    const USERNAME_REGEX = "validations.username_regex";

    const FULL_NAME_REGEX = "validations.full_name_regex";
    const VALIDATION_MIMES = "validations.mimes";
    const VALIDATION_SAME = "validations.same";

    const VALIDATION_IMAGE_UPLOADED = "validations.uploaded";
    const ERROR_OCCURRED = "messages.errorOccurred";
    const CHECK_PERMISSION = "App\Http\Middleware\CheckPermission:";
    const TRANSLATION_TABLE = "_translations";
    const MAX_30 = 'max:30';

    const ZERO_ZERO = " 00:00:00";

    const FULL_FULL = " 23:59:59";

    const API_SUCCESS =  "api.success";

    const CMS_MANGER_EDIT =  'App\Http\Middleware\CheckPermission:CMS_MANGER_EDIT';
    const EMAIL_TEMPLATE_LIST =  'App\Http\Middleware\CheckPermission:EMAIL_TEMPLATE_LIST';
    const EMAIL_TEMPLATE_CHANGE_STATUS =  'App\Http\Middleware\CheckPermission:EMAIL_TEMPLATE_CHANGE_STATUS';
    const EMAIL_TEMPLATE_ADD =  'App\Http\Middleware\CheckPermission:EMAIL_TEMPLATE_ADD';
    const EMAIL_TEMPLATE_EDIT =  'App\Http\Middleware\CheckPermission:EMAIL_TEMPLATE_EDIT';
    ...
    const SELECT_COUNTRY_CODE = "messages.selectCountryCode";
    const COMMUNITIES = ['1' => 'Public', 2 => 'Private', 3 => 'Hidden', 0 => 'N/A'];
    const VIDEO_CATEGORY = ['1' => 'Free', 2 => 'Paid'];
    const VIDEO_SERVICE = ['' => 'Select Video service', 'free' => 'Free', 'paid' => 'Paid'];
    const SONGS_CATEGORY = ['' => 'Select Songs service', 'free' => 'Free', 'paid' => 'Paid'];
    const SUBSCRIPTION_CATEGORY = ['' => 'Select Subscription Type', 1 => 'Overall Subscription', 2 => 'Meditation Subscription'];

    const MIN_2 =  "min:2";

    const REPORT_REASON = ['1' => 'Nudity', 2 => 'Violence', 3 => 'Offensive'];
    const S3_URL = 'https://octal-dev-ghtr.s3.ap-south-1.amazonaws.com/';

    const ALLOW_SUB_ADMIN_PASSWORD_CREATE = true;
    const MANUAL_FACTOR = 'Manual';
    const SYSTEM_FACTOR = 'System';

    const ALLOW_SUB_ADMIN_PHONE_NO = true;

    const DB_DATE_FORMAT = 'Y-m-d H:i:s';

    const DB_ONLY_DATE_FORMAT = 'Y-m-d';

    const USER_EXCLUDE_FIELDS = ['id', 'email', 'role_id', 'password', 'full_name'];
    const DB_BKP = 'db_bkp';
    ...
    const SELECT_TIMEZONE ='Select Timezone';
    const TWO_FACTOR_AUTHENTICATION = [''=>'Select Two Way','Yes'=>'Yes','No'=>'No'];

    const PASSWORD_GENERATION = [''=>'Select Password Generation','Manual'=>'Manual','System'=>'System'];

    const SUB_ADMIN_MANAGEMENT = 'Sub-Admin Management';
    const USER_MANAGEMENT = 'User Management';
    const STATUS_UPDATED = 'Status Updated';
    const TESTIMONIAL_MANAGEMENT = 'Testimonial Management';
    const BANNER_MANAGEMENT = 'Banner Management';

    const PORTFOLIO_MANAGEMENT = 'Portfolio Management';

    const GALLERY_MANAGEMENT = 'Gallery Management';
    const VIDEO_MANAGEMENT = 'Video Management';
    const AUDIO_MANAGEMENT ='Audio Management';
    const EMAIL_MANAGEMENTS ='Email Management';
    const CMS_MANAGEMENT = 'CMS Management';
    const SETTING_MANAGEMENT ='Setting Management';
    const ROLE = 'Role';
    const COUPON_MANAGEMENT ='Coupon Management';
    const META_MANAGEMENT ='Meta Management';

    const GET_SECTION = 'get_section:';
    const MIN_1 = 'min:1';
    const FLOT_NUMBER_REGEX = 'regex:/^\d*\.?\d+$/';

    const OTP_VERIFICATION_MODE = [''=>'Select Otp Verification Mode','Test'=>'Test','Production'=>'Production'];
    const OTP_TEST_MODE = 'Test';
    const OTP_PRODUCTION_MODE = 'Production';
    const OFFER_TYPE = [''=>'Select Offer Type','percentage' => 'Percentage', 'flat' => 'Flat'];
