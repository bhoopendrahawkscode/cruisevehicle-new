...
error_reporting(E_ALL);
        ...
        $this->options = array(
            ...
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME'),2).'/storage/app/public/uploads/media/',
            'upload_url' => $this->get_full_url().'/storage/uploads/media/',
    ...
    public function post($print_response = true) {
		...
		if(isset($files[0]->thumbnailUrl) && !empty($files[0]->thumbnailUrl)){ //pGupta
            $title = basename($files[0]->name, '.' . pathinfo($files[0]->name, PATHINFO_EXTENSION));
			MediaManager::create(["name"=>$title,"image"=>$files[0]->name]);
		}
