...
<div class="header d-flex align-items-center">
    <h1 class="page-header">
        {{ trans('messages.subAdmin') }} {{ trans('messages.list') }}
    </h1>
    <ol class="breadcrumb ms-auto mb-0">
        <li><a href="{{ Route('admin.dashboard') }}"><em class="fa fa-dashboard"></em> {{ trans('messages.dashboard') }} </a></li>
        <li class="active">{{ trans('messages.subAdmin') }} {{ trans("messages.list") }}</li>
    </ol>
</div>
<div id="page-inner">
    <div class="panel panel-default">
		<div class="panel-body form_mobile">
             {{ html()->modelForm(null, 'GET')->route('admin.listSubadmin')
			...
			<div class="form_row">
				<div class="form_fields d-flex mb-0 gap-2">
				 ...
				 <div class="form-action ">
                    ...
                    <a href="{{ Route('admin.listSubadmin') }}" class="btn btn-sm border_btn btnIcon" title="{{ trans('messages.reset') }}"><em class='fa fa-refresh '></em> </a>
                    @can('SUB_ADMIN_ADD')
                    <a href="{{ Route('admin.Subadmin_add') }}" class="btn theme_btn bg_theme btn-sm py-2   btnIcon" style="margin:0;" title="{{ trans('Add New Sub-Admin') }}"> <em class='fa fa-add '></em></a>
                    @endcan
                ...
                <table class="@if (!$result->isEmpty()) table table-striped table-hover @endif">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                {{-- <a title="Sub-Admin Login"  href="{{ URL::to('admin/subadmin-login/' . $record->id) }}"  class="btn btn-primary">
                                    <em class="fa fa-unlock"></em>
                                </a> --}}
                                @can('SUB_ADMIN_EDIT')
                                <a title="{{ trans('Edit') }}" href="{{ URL::to('admin/sub-admin-list/edit/' . $record->id) }}" class="btn btn-primary">
                                    <em class="fa fa-pencil"></em>
                                </a>
                                @endcan
                                {{--
                                    @can('SUB_ADMIN_DELETE')
                                        <a class="btn btn-primary delete_any_item" data-id="{{ route('admin.user.sub_admin.delete',$record->id) }}" title="{{ trans('Delete') }}" href="javascript::void(0);">
