			...
			<div class="form_row">
				<div class="form_fields d-flex mb-0 gap-2">
					<div class="form-group mb-0">
                        ...
                        'placeholder' => trans("messages.searchBy")." ".trans('messages.quote')]) }}
					</div>
					<div class="form-group mb-0 w-20p">
                        {{ html()->select('day',['1' => '01', '2' => '02','3' => '03','4' => '04','5' => '05','6' => '06','7' => '07'],
						((isset($searchVariable['day'])) ? $searchVariable['day'] : ''))
                        ->attributes(['class' => 'form-control form-control-dropdown'])->placeholder(trans("messages.day")) }}
					</div>
					...
					<thead>
                        ...
                        <th scope="col" class="w-15p">
							{{
                                    link_to_route(
                                        $listRoute,
                                        trans("messages.writtenBy"),
                                        array(
                                            'sortBy' => 'written_by',
                                            'order' => ($sortBy == 'written_by' && $order == 'desc') ? 'asc' : 'desc'
                                        ),
                                        $query_string
                                    )
								}}
							@php getSortIcon($sortBy,$order,'written_by') @endphp
						</th>
                        ...
                        <th scope="col" class="w-5p">
							{{
                                    link_to_route(
                                        ...
                                        array(
                                            'sortBy' => 'day',
                                            'order' => ($sortBy == 'day' && $order == 'desc') ? 'asc' : 'desc'
                                        ),
                                        $query_string
                                    )
								}}
							@php getSortIcon($sortBy,$order,'day') @endphp
						</th>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								...
								<a style='display:none' title="{{ trans('messages.delete') }}" href="{{URL::to('admin/'.$listUrl.'/delete/'.$record->id)}}" class="btn btn-danger">
