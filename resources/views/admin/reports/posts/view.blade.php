...
<div class="header d-flex align-items-center">
	<h1 class="page-header">
		{{ trans("messages.reportedPost") }} {{ trans("messages.list") }} >> {{ trans("messages.whoReported") }} ({{excerpt($blogDetails['content'],100)}})
	</h1>
	<ol class="breadcrumb ms-auto mb-0">
		<li><a href="{{ Route('admin.dashboard') }}">
				<em class="fa fa-dashboard"></em>
				{{ trans("messages.dashboard") }}</a></li>
        <li><a href="{{ Route('admin.reportPostlist') }}">
            <em class="fa fa-blog"></em>
            {{ trans("messages.reportedPost") }} {{ trans("messages.list") }} </a></li>
...
<div id="page-inner">
    <div class="panel panel-default">
        <div class="panel-body form_mobile">
            {{ html()->modelForm(null, 'GET')->route('admin.reportPost.view',$id)
         ->attributes(['id'=>'posts','class'=>'form-inline','role'=>'form','autocomplete' => 'off','onSubmit'=>'return checkDate();'])->open() }}
            {{ html()->hidden('display') }}
            <div class="form_row">
                ...
                <div class="form-action ">
                    ...
                    <a title="{{ trans('messages.reset') }}" href="{{Route('admin.reportPost.view',$id)}}" class="btn btn-sm border_btn btnIcon">
		...
		<div class="panel-body">
			...
			<div class="table-responsive">
				<table class="@if (!$result->isEmpty()) table table-striped table-hover @endif" id="sortable">
					<caption>{{trans("messages.reportedPost")}} {{ trans("messages.view") }}</caption>
                        ...
                        <th scope="col" class="w-10p">
							{{
                                    link_to_route(
                                        ...
                                        array(
                                            'id'=>$id,
                                            'sortBy' =>  'blog_reports.created_at',
                                            'order' => ($sortBy == 'blog_reports.created_at' && $order == 'desc') ? 'asc' : 'desc'
                                        ),
                                        $query_string
                                    )
								}}
							@php getSortIcon($sortBy,$order,'blog_reports.created_at') @endphp
						</th>
                        <th scope="col" class="w-10p">
                            {{ trans("messages.type") }}
                        </th>
