			...
			<div class="form_row">
				<div class="form_fields d-flex mb-0 gap-2">
					<div class="form-group mb-0">
                        {{ html()->text('name',((isset($searchVariableBlog['name'])) ?
						$searchVariableBlog['name'] : ''))
                        ->attributes(['class' => 'form-control',
                        'placeholder' => trans("messages.searchBy")." ".trans('messages.username')]) }}
					</div>
                    <div class="form-group mb-0 w-20p">
                        {{ html()->select('type',Constant::COMMUNITIES,
						((isset($searchVariableBlog['type'])) ? $searchVariableBlog['type'] : ''))
					...
					<div class="form-group mb-0 calendarIcon">
                        {{ html()->text('from',((isset($searchVariableBlog['from'])) ?
                        $searchVariableBlog['from'] : ''))
                        ->attributes(['class' => 'form-control datepicker','onkeydown'=>'return false;',
                        'placeholder' => trans('messages.fromDate')]) }}
					</div>
					<div class="form-group mb-0 calendarIcon">
						{{ html()->text('to',((isset($searchVariableBlog['to'])) ?
                        $searchVariableBlog['to'] : ''))
                        ->attributes(['class' => 'form-control datepicker','onkeydown'=>'return false;',
				...
				<table class="@if (!$result->isEmpty()) table table-striped table-hover @endif" id="sortable">
					...
					<thead>
                        ...
                        <th scope="col" class="w-10p">
                            <?php
                                ...
                                if(isset($_GET['type']) && $_GET['type'] ==0) { echo trans("messages.communityName"); ?>
                            ...
                            {{ getSortIcon($sortBy,$order,'communities.name'); }}
                            @php } @endphp
						</th>
                        ...
                        <th scope="col" class="w-10p">
                            <?php
                                ...
                                if(isset($_GET['type']) && $_GET['type'] ==0) { echo trans("messages.communityType"); ?>
                            ...
                            {{ getSortIcon($sortBy,$order,'communities.type'); }}
                            @php } @endphp
						</th>

                        <th scope="col" class="w-30p">
							{{
                                    link_to_route(
                                        $listRoute,
                                        trans("messages.post"),
                                        array(
                                            'sortBy' =>  $mainTable.'.content',
                                            'order' => ($sortBy ==  $mainTable.'.content' && $order == 'desc') ? 'asc' : 'desc'
                                        ),
                                        $query_string
                                    )
								}}
							@php getSortIcon($sortBy,$order,$mainTable.'.content') @endphp
						</th>
					...
					<tbody>
						...
						<tr>
                            ...
                            <td>@can($viewPermission)
                                    <a title="{{ trans('messages.view') }}" href="{{URL::to('admin/'.$listUrl.'/view/'.$record->id)}}" >
                                        {{excerpt($record->content,100)}}
                                    </a>
                                    @else
                                    {{excerpt($record->content,100)}}
                                    @endcan
                            ...
                            <td>
                                @can("POSTS_MANAGEMENT_CHANGE_STATUS")
                                    @if ($record->status == 1)
