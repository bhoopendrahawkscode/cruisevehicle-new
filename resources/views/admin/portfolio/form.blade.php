    ...
    <div class="tab-content" id="tab-parent">
        ...
        <div language="{{ $languageRow->id }}" class="tab-pane   fade @php if($j == 0){echo " active show"; } @endphp" id="custom-tabs-{{ $languageRow->id }}" role="tabContent">
            ...
            <div class="row">
                <div class="col-12 col-sm-6">
                    <div class="clearValidation form-group
                            {{ $errors->has('url[' . $languageRow->id . ']') ? 'has-error': '' }}">
                        <label for=" {{ trans('messages.url') }} {{ '(' . $languageRow->name . ')' }}">
                            {{ trans('messages.url') }} {{ '(' . $languageRow->name . ')' }}
                            <span class="red_lab"> *</span>
                        </label>
                        {{ html()->text('url[' . $languageRow->id . ']')->attributes(['class' => 'form-control languages_url formValidate',
                            'placeholder' => trans("messages.url")]) }}
                        @if ($errors->has('url.' . $languageRow->id))
                        <span class="invalid-feedback " role="alert">
                            <strong>{{ $errors->first('url.' . $languageRow->id) }}</strong>
                        </span>
                        @endif
                    </div>
                </div>
