<div class="row">
    <div class="col-md-6">
        <div class="form-group clearValidation3">
            <label for="brand_id">{{ trans('messages.brand') }} @lang('messages.list')
    ...
    <div class="col-md-6">
        <div class="form-group">
            <label for="name">{{ trans('Model Name') }} <span class="red_lab">
                    *</span></label>
            {{ html()->text('name', null)->attributes(['class' => 'form-control', 'placeholder' => trans('Model Name'), 'enctype' => 'multipart/form-data'])->open() }}
            @if ($errors->has('name'))
                <span class="invalid-feedback" role="alert">
...
@if (env('ENABLE_CLIENT_VALIDATION'))
    <script>
        ...
        $(function() {
            ...
            window.formReference = $("#modelForm").validate({
                rules: {
                    name: {
                        required: true,
                        minlength: 2,
