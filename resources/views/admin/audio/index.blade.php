...
@section('content')
            ...
            <div class="panel-body form_mobile">
                ...
                <div class="form_row">
                    <div class="form_fields d-flex mb-0 gap-2">
                        <div class="form-group mb-0">
                            {{ html()->text('name', isset($searchVariable['name']) ? $searchVariable['name'] : '')->attributes([
                                    'class' => 'form-control',
                                    'placeholder' => trans('messages.searchBy') . ' ' . trans('messages.artist'),
                                ]) }}
                        </div>
                    ...
                    <div class="form-action ">
                        ...
                        <a href="{{ url('admin/export/Audio/xls') }}" class="btn btn-sm border_btn"
                            title="Export Xls">Export Xls</a>
                        <a href="{{ url('admin/export/Audio/csv') }}" class="btn btn-sm border_btn"
                    ...
                    <table class="@if (!$result->isEmpty()) table table-striped table-hover @endif" id="sortable">
                        ...
                        @if (!$result->isEmpty())
                            <thead>
                                ...
                                <th scope="col" class="w-5p">
                                    {{ trans('messages.audio') }}
                                </th>
                                <th scope="col" class="w-10p">
                                    {{ trans('messages.image') }}
                                </th>
                                <th scope="col" class="w-10p">
                                    {{ trans('messages.numberOfTimes') }}
                                </th>
                                <th scope="col" class="w-10p">
                            ...
                            @if (!$result->isEmpty())
                                @foreach ($result as $key => $record)
                                    <tr>
                                        ...
                                        <td>
                                            <a data-src="{{ $record->audio }}" onclick="showAudio(this);"
                                                href="javascript:void(0);">{{ __('messages.play') }}</a>
                                        </td>
                                        <td>
                                            <img class="border border-1" alt="{{ $record->name }}"
                                                src="{{ $record->thumbImage }}" width="50">
                                        </td>
                                        <td>
                                            <?php
                                            if (isset($audioHistoryCounts[$record->id])) {
                                                echo $audioHistoryCounts[$record->id];
                                            } else {
                                                echo 0;
                                            }
                                            ?>
    ...
    <script>
        function showAudio(j) {
            let audioUrl = $(j).attr('data-src');
    ...
    <div class="modal fade" id="videoPreview" tabindex="-1" role="dialog" aria-labelledby="videoPreviewTitle"
        aria-hidden="true">
        <div class="modal-dialog" role="document" style="max-width: 335px;width: 100%;">
