                ...
                <div class="form_row">
                    ...
                    <div class="form-action ">
                        ...
                        <a href="{{ route($listRoute) }}" class="btn btn-sm border_btn btnIcon"
                            ...
                            {{-- @can('SUB_ADMIN_ADD')
                            <a href="{{route('admin.vehicle.add')}}" class="btn theme_btn bg_theme btn-sm py-2  btnIcon"
                ...
                <table class="@if (!$result->isEmpty()) table table-striped table-hover @endif">
                    ...
                    @if (!$result->isEmpty())
                        <thead>
                            ...
                            <th scope="col">
                                {{ trans('messages.fuel_used') }}

                            </th>

                            <th scope="col">
                                {{ trans('messages.engine_capacity') }}

                            </th>

                            <th scope="col">
                                {{ trans('messages.transmission_type') }}
                        ...
                        @if (!$result->isEmpty())
                            @foreach ($result as $key => $record)
                                <tr>
                                    ...
                                    <td>
                                        {{ $record['engineCapacity']->capacity ?? '' }}
                                    </td>
                                    <td>
                                        {{ $record['transmissionType']->name ?? '' }}
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        {{-- @can('SUB_ADMIN_EDIT')
                                            <a title="{{ trans('Edit') }}" href="{{route('admin.vehicle.edit',$record->id)}}" class="btn btn-primary">
                                                <em class="fa fa-pencil"></em>
                                            </a>
                                        @endcan --}}
                                        @can('VEHICLE_VIEW')
                                        <a title="{{ trans('Edit') }}" href="{{route('admin.vehicle.view',$record->id)}}" class="btn btn-warning ">
                                            <em class="fa fa-eye"></em>
                                        </a>
                                    @endcan
                                        {{-- @can('SUB_ADMIN_DELETE')
                                            <a class="btn btn-primary delete_any_item" data-id="{{route('admin.vehicle.delete',$record->id)}}"
