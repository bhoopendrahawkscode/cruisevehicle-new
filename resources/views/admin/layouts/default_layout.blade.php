...
<html lang="en">
    <head>
        ...
        if(isset($section) && !empty($section)){ ?>
            <title>{{ GeneralService::getSettings('websiteTitle') }} - <?php echo $section; ?></title>
        <?php }else{ ?>
            <title>{{ GeneralService::getSettings('websiteTitle') }}</title>
        <?php } ?>
        ...
        <meta content="{{ GeneralService::getSettings('websiteTitle') }} <?php if($version != ''){ echo $version; } ?>" name="description" />

        <link href="{{ ImageService::getImageUrl(Config::get('constants.LOGO_FOLDER') . GeneralService::getSettings('favicon')) }}" rel=icon sizes=32x32 type=image/png>
        <link href="{{ ImageService::getImageUrl(Config::get('constants.LOGO_FOLDER') . GeneralService::getSettings('favicon')) }}" rel=icon sizes=16x16 type=image/png>
        <link href="{{ ImageService::getImageUrl(Config::get('constants.LOGO_FOLDER') . GeneralService::getSettings('favicon')) }}" rel=apple-touch-icon>
        <link rel="shortcut icon" type="image/x-icon" href="{{ ImageService::getImageUrl(Config::get('constants.LOGO_FOLDER') . GeneralService::getSettings('favicon')) }}">

        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
        {{-- <script src="https://kit.fontawesome.com/2b392c3eb3.js" crossorigin="anonymous"></script> --}}
		...
		<script src="{{ asset ('/assets/js/jquery.validate.min.js') }}?id=<?php echo random_int(0,9);  ?>"></script>
		<script src="{{ asset ('/assets/js/jquery.validate_custom.min.js') }}?id=<?php echo random_int(0,9);  ?>"></script>
		...
		<link href="{{ asset('/assets/css/custom-styles.css') }}?id=<?php echo random_int(0,9);  ?>" rel="stylesheet" />
...
//window.onload = adjustWidthBasedOnPlaceholder;
		  ...
		  <div id="page-wrapper">
			...
			<div class="offcanvas offcanvas-end" data-bs-scroll="true" data-bs-backdrop="false" tabindex="-1" id="offcanvasScrolling" aria-labelledby="offcanvasScrollingLabel">
				<div class="offcanvas-header">
					<h5 class="offcanvas-title" id="offcanvasExampleLabel">{{__('messages.themeSettings')}}</h5>
	...
	<script type="text/javascript">
        ...
        function saveTheme(type,value){
            ...
            var routeUrl = "{{ route('admin.updateCss') }}";
            $.ajax({
                url: routeUrl,
                method: 'POST',
                data: {
            ...
            <script>
                $(document).ready(function () {
                    $('.status_any_item').on('change', function() {
                        ...
                        var location= "{!! URL::to($statusChangeUrl) !!}"+'/'+id+'/'+$(this).attr('status');
                        Swal.fire({
