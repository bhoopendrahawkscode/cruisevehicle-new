                ...
                <div class="form_row">
                    <div class="form_fields d-flex mb-0 gap-2">
                        <div class="form-group mb-0">
                            {{ html()->text('capacity', isset($searchVariable['capacity']) ? $searchVariable['capacity'] : '')->attributes(['class' => 'form-control',
                    ...
                    <div class="form-action ">
                        ...
                        <a href="{{ route($listRoute) }}" class="btn btn-sm border_btn btnIcon"
                            ...
                            @can('ENGINE_CAPACITY_ADD')
                            <a href="{{route('admin.enginecapacity.add')}}" class="btn theme_btn bg_theme btn-sm py-2  btnIcon"
                    ...
                    @if (!$result->isEmpty())
                        <thead>
                            ...
                            <th scope="col">
                                ...
                                @php getSortIcon($sortBy,$order,'capacity') @endphp
                            </th>
                            <th scope="col">
                                {{ trans('Brand name')}}
                        ...
                        @if (!$result->isEmpty())
                            @foreach ($result as $key => $record)
                                <tr>
                                    ...
                                    <td>
                                    {{ $record->brand ? $record->brand->name : 'N/A' }}
                                     
                                    </td>
                                    <td>
                                    {{ $record->model ? $record->model->name : 'N/A' }}
                                    ...
                                    <td>
                                        @can('ENGINE_CAPACITY_CHANGE_STATUS')
                                @if ($record->status == 1)
                                ...
                                @endcan
                                    ...
                                    <td>
                                        @can('ENGINE_CAPACITY_EDIT')
                                            <a title="{{ trans('Edit') }}" href="{{route('admin.enginecapacity.edit',$record->id)}}" class="btn btn-primary">
                                                <em class="fa fa-pencil"></em>
                                            </a>
                                        @endcan
                                        @can('ENGINE_CAPACITY_DELETE')
                                            <a class="btn btn-primary delete_any_item" data-id="{{route('admin.enginecapacity.delete',$record->id)}}"
