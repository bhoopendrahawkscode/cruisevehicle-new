		...
		<div class="panel-body form_mobile">
			...
			<div class="form_row">
				...
				<div class="form-action ">
                    ...
                    <a href="{{url('admin/export/Banner/xls')}}" class="btn btn-sm border_btn" title="Export Xls">Export Xls</a>
                    <a href="{{url('admin/export/Banner/csv')}}" class="btn btn-sm border_btn" title="Export Csv">Export Csv</a>
				...
				<table class="@if (!$result->isEmpty()) table table-striped table-hover @endif" id="sortable">
					...
					<tbody>
						...
						<tr>
							...
							<td class="breakAll">
							@if (!empty($record->banner_link))
								<a href="{{ $record->banner_link }}" target="_blank" class="inline-block px-4 py-2 text-sm font-medium text-info bg-blue-600 rounded hover:bg-blue-700 transition-all duration-200">
									Click here
								</a>
							@else
							...
							<td>
                                ...
                                <a class="btn btn-primary delete_any_item d-none" data-id="{{ URL::to('admin/'.$listUrl.'/delete/' . $record->id) }}" title="{{ trans('Delete') }}" href="javascript::void(0);">
