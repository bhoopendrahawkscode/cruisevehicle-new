    ...
    <div class="tab-content" id="tab-parent">
        ...
        <div language="{{ $languageRow->id }}" class="tab-pane   fade @php if($j == 0){echo " active show"; } @endphp" id="custom-tabs-{{ $languageRow->id }}" role="tabContent">
            <div class="row">
                <div class="col-12 col-sm-6">
                    <div class="clearValidation form-group
                        ...
                        <label for=" {{ trans('messages.title') }} {{ '(' . $languageRow->title . ')' }}">
                            {{ trans('messages.title') }} {{ '(' . $languageRow->name . ')' }}
                            <span class="red_lab"> *</span>
                        </label>
                        {{ html()->text('title[' . $languageRow->id . ']')->attributes(['class' => 'form-control languages_title formValidate',
            ...
            <div class="row">
                <div class="col-12 col-sm-6">
                    <div class="clearValidation form-group
                            {{ $errors->has('banner_link[' . $languageRow->id . ']') ? 'has-error': '' }}">
                        <label for=" {{ trans('Link') }} {{ '(' . $languageRow->banner_link . ')' }}">
                            {{ trans('Banner Link') }} {{ '(' . $languageRow->name . ')' }}
                            <span class="red_lab"> *</span>
                        </label>
                        {{ html()->text('banner_link[' . $languageRow->id . ']')->attributes(['class' => 'form-control languages_banner_link formValidate',
                            'placeholder' => trans("Banner Link")]) }}
                        @if ($errors->has('banner_link.' . $languageRow->id))
                        <span class="invalid-feedback " role="alert">
                            <strong>{{ $errors->first('banner_link.' . $languageRow->id) }}</strong>
                        </span>
                        @endif
                    </div>
                </div>
